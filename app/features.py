...
def extract_features_for_horse(
    horse_details: Dict, race_info: Dict = None, entry_info: Dict = None
) -> Dict[str, float]:
    """
    ...
    # 複合指標
    features["strong_record"] = (
        features["win_rate"] * 0.5 + features["place_rate"] * 0.3 + features["show_rate"] * 0.2
    )
    features["consistency"] = (
        features["place_rate"] / (features["win_rate"] + 0.01) if features["win_rate"] > 0 else 0
    )
    ...
    features["pedigree_score"] = (
        features["sire_win_rate"] * 0.6 + features["dam_sire_win_rate"] * 0.4
    )
