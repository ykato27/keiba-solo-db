...
class DataLeakageValidator:
    ...
    def validate_timeseries_split(
        ...
        for cls in unique_classes:
            ...
            train_dist[int(cls)] = {"count": int(train_count), "percentage": round(train_pct, 2)}
            test_dist[int(cls)] = {"count": int(test_count), "percentage": round(test_pct, 2)}
        ...
        if missing_in_train:
            ...
            validation_result["status"] = (
                "✅ クラス分布OK: テストセットのすべてのクラスが訓練セットに含まれています"
            )
    ...
    def validate_cv_splits(
        ...
        Returns:
            複数分割の検証結果
        """
        results = {"total_folds": len(cv_splits), "all_valid": True, "folds": [], "summary": {}}
    ...
    def check_feature_leakage(
        feature_names: List[str], excluded_patterns: List[str] = None
    ) -> Dict[str, Any]:
        """
        機能レベルでのリーク検出
        （未来情報を含む可能性のある特徴量を警告）
    ...
    def validate_entry_completeness(
        X: np.ndarray, entries_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        エントリデータの完全性検証
        ...
        if entries_data:
            ...
            result["finish_pos_coverage"] = (
                (finish_pos_count / len(entries_data) * 100) if entries_data else 0
            )
    ...
    def print_validation_report(cv_results: Dict[str, Any]) -> None:
        ...
        print("\n" + "-" * 80)
        print(
            f"サマリー: {cv_results['summary']['total_valid_folds']}/{cv_results['total_folds']} Fold が有効"
        )
