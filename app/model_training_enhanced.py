...
class ModelTrainingEnhanced:
    ...
    @staticmethod
    def analyze_learning_curve(train_losses: List[float], val_losses: List[float]) -> Dict:
        """
        ...
        if len(train_losses) < 10:
            return {"status": "⚠️ サンプル不足", "message": "10ラウンド以上の訓練が必要です"}
        ...
        if gap < 0.05:
            ...
            diagnosis["recommendation"] = (
                "max_depth を削減、learning_rate を下げる、n_estimators を削減"
            )
        else:
    ...
    def compute_fold_wise_metrics(
        ...
        for fold_idx, (y_true, y_pred, y_proba) in enumerate(
            zip(y_true_list, y_pred_list, y_pred_proba_list), start=1
        ):
            # 各クラスごとの精度
    ...
    def generate_training_report(
        training_history: Dict, fold_metrics: Dict, learning_curve_diagnosis: Dict
    ) -> Dict:
        """
        訓練レポートを生成
    ...
    def _generate_recommendations(fold_metrics: Dict, overfitting_diag: Dict) -> List[str]:
        ...
        if std_acc > 0.1:
            recommendations.append(
                "🟡 Fold 間の精度に大きなばらつきあり。データの時系列構造を再確認"
            )
